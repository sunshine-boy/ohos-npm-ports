...
- **`package.json`**：`name` → `@tetcl/node-pre-gyp`，`version` → `2.0.3-4`，`repository` / `bugs` 与 **`ports/js-native`** 一致（`git+https://github.com/sunshine-boy/ohos-npm-ports.git` / `https://github.com/sunshine-boy/ohos-npm-ports/issues`）。
