...
- **`package.json`**：`name` → `@ohos-npm-ports/node-pre-gyp`，`version` → `2.0.3-1`，`repository` / `bugs` → 本维护仓库。

业务侧若仍依赖包名 `@mapbox/node-pre-gyp`，可使用 npm **`overrides`** 或改为依赖 **`@ohos-npm-ports/node-pre-gyp`**，并相应调整子依赖中的 `peerDependencies`（如有）。
